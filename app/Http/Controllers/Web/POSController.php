...
$todayOrderTotal = (int) \App\Models\Order::whereDate('business_date', today())->sum('total')
                         + (int) \App\Models\TempOrder::whereDate('business_date', today())->sum('total');
        ...
        try {
            ...
            if ($orderLimitEnabled) {
                ...
                if ($now >= $limitStart && $now <= $limitEnd) {
                    $orderLimitAmount = (int) \App\Models\Setting::get('order_limit_amount', '0');
                    $todayTotal = (int) \App\Models\Order::whereDate('business_date', today())->sum('total')
                                + (int) \App\Models\TempOrder::whereDate('business_date', today())->sum('total');
